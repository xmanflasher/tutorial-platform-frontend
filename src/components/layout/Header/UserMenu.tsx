...
import { User, LogOut, Moon, User as UserIcon, Users } from 'lucide-react';
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      <button 
        ...
        className="w-10 h-10 rounded-full overflow-hidden border-2 border-slate-700 hover:border-slate-500 transition-colors"
      >
        {user.avatar ? (
            // 這裡用 img 簡單處理，正式環境建議用 next/image
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-72 bg-[#1e232e] border border-slate-700 rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
          ...
          {/* 2. 選單選項 */}
          <div className="p-2">
            <Link href="/profile" className="flex items-center gap-3 px-4 py-3 text-slate-300 hover:bg-slate-700/50 hover:text-white rounded-lg transition-colors">
              <UserIcon size={18} />
              <span>個人檔案</span>
            </Link>
